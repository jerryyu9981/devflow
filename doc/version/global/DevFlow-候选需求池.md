...
### v2.9.0 候选（质量保障强化版本 + 全自动循环架构）

| ID | 需求描述 | 优先级 | 来源 | 技术可行性 | 状态 | 目标版本 |
|:--:|:---------|:------:|:-----|:----------:|:----:|:--------:|
| V260-039 | **重试管理器（retry-manager）**——嵌入到 devflow-phase-manager 的阶段切换门禁中，为每个 Step 增加重试计数和上限 | 🔴 P0 | 全自动循环 | ⚠️ 需预研 | 📋 候选 | v2.9.0 |
| V260-040 | **空池检测器（empty-pool-detector）**——在 Step 5 闭环后检测候选需求池状态 | 🔴 P0 | 全自动循环 | ✅ 可行 | 📋 候选 | v2.9.0 |
| V260-041 | **自动降级机制（auto-degrade）**——为测试覆盖率略低等"灰色地带"提供自动通过逻辑 | 🟡 P1 | 全自动循环 | ⚠️ 需预研 | 📋 候选 | v2.9.0 |
| V260-042 | **致命错误处理器（fatal-error-handler）**——贯穿所有 Step，捕获不可恢复的错误 | 🟡 P1 | 全自动循环 | ✅ 可行 | 📋 候选 | v2.9.0 |
| V260-043 | **循环状态机（cycle-state-machine）**——新增 orchestrator 技能，管理"版本间循环"生命周期 | 🟡 P1 | 全自动循环 | ⚠️ 需预研 | 📋 候选 | v2.9.0 |
| V290-001 | **还债配额机制**——每个版本必须用 15~20% 工作量偿还历史债务，否则不通过 Step 0 评审 | 🟡 P1 | 质量保障 | ✅ 可行（流程规则 + 评审门禁） | 📋 候选 | v2.9.0 |
| V290-002 | **跨版本债务流转机制**——建立债务继承、升级、偿还、挂起的完整生命周期管理 | 🟡 P1 | 质量保障 | ✅ 可行（流程规则 + 状态字段） | 📋 候选 | v2.9.0 |
| V290-003 | **测试覆盖率门禁**——核心模块行覆盖率≥80%、分支覆盖率≥70%，未达标阻塞 Step 3 完成 | 🟡 P1 | 质量保障 | ✅ 可行（工具集成 + 阈值配置） | 📋 候选 | v2.9.0 |
| V290-004 | **端到端集成验证**——涉及两个以上模块/服务时，必须有至少一条端到端验证用例 | 🟡 P1 | 质量保障 | ✅ 可行（流程规则 + 模板） | 📋 候选 | v2.9.0 |
...
| V260-036-07 | devflow-init 版本差异检测增强 | 🔴 P0 | 三阶段架构 | ✅ 可行 | 📋 候选 | 待分配 |
| V260-036-08 | 填充 version.json 仓库地址字段 | 🟢 P2 | 三阶段架构 | ✅ 可行 | 📋 候选 | 待分配 |
