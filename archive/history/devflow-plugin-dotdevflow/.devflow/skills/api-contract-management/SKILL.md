...
当项目无 OpenAPI 规范，或作为契约测试的补充手段时，维护一张"路由→请求/响应字段"映射表（`route-mapping.yaml`）。通过自动化 diff 脚本对比后端路由定义与前端 API 调用代码，在 Step 4 测试阶段提前发现前后端接口漂移问题。
...
| 字段 | 类型 | 说明 |
|:-----|:-----|:-----|
| `path` | string | API 路由路径，含版本前缀 |
| `method` | string | HTTP 方法（GET/POST/PUT/DELETE/PATCH） |
| `request_fields` | array | 请求字段列表，每项含 `name`/`type`/`required` |
| `response_fields` | array | 响应字段列表，每项含 `name`/`type` |
| `frontend_call` | string | 前端调用位置，格式为 `文件路径#函数名` |

**diff 脚本规范**：

| 项目 | 说明 |
|:-----|:-----|
| **输入** | ① 后端路由定义（FastAPI 路由代码 / OpenAPI spec） ② 前端 API 调用代码（`src/api/` 下的 `.ts`/`.tsx`/`.vue` 文件） |
| **输出** | 差异清单（Markdown 格式），包含 4 类差异分类 |
| **执行方式** | 命令行执行：`npx tsx scripts/route-mapping-diff.ts --backend ./backend --frontend ./frontend/src/api --output ./diff-report.md` |

**diff 流程**：

1. **从后端路由定义提取映射**：解析 FastAPI 路由装饰器（或从 `openapi.json` 生成），提取每个路由的 `path`/`method`/`request_fields`/`response_fields`
2. **从前端 API 调用代码提取映射**：扫描前端 API 调用文件，提取实际使用的路由路径、HTTP 方法、请求参数和响应字段消费情况
3. **diff 两份映射**：逐路由对比后端定义与前端调用，输出差异清单
4. **输出差异清单**，包含以下 4 类差异：

| 差异类型 | 编号 | 说明 | 示例 |
|:---------|:----:|:-----|:-----|
| 后端有前端未调用 | D1 | 后端定义了路由但前端代码中无对应调用 | 后端有 `DELETE /api/v1/users/{id}`，前端从未调用 |
| 前端调用后端未定义 | D2 | 前端调用了路由但后端无定义 | 前端调用 `PUT /api/v1/users/avatar`，后端无此路由 |
| 字段名/类型不一致 | D3 | 同一路由的请求或响应字段名或类型不匹配 | 后端返回 `user_name`（string），前端期望 `username`（string） |
| 必填规则不一致 | D4 | 同一字段在后端和前端的必填属性不一致 | 后端 `email` 必填，前端调用时未传 |

**执行时机**：Step 4 测试阶段执行（非仅 Step 5 CI），测试报告中记录 diff 结果。P0/P1 路由差异须在测试通过前修复或明确豁免。
