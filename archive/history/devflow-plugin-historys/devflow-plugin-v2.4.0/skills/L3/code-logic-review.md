...
### 12. 偏执级检查（VR-2.4.1-003 新增）

以下三项为偏执级（paranoid）检查，针对常见但易被忽略的深层问题。即使在常规审查中未发现异常，也应主动扫描这些模式。

#### 12.1 N+1 查询检测

检查：

- 循环内部是否存在数据库查询、API 调用或文件 I/O 操作
- 批量操作是否使用批量查询/批量插入替代逐条操作
- ORM 或查询构建器是否导致隐式 N+1（如延迟加载关联数据）
- 列表渲染是否对每个 item 触发独立请求

发现 N+1 模式时，应要求重构为批量操作或预加载，并记录预期性能改善。

#### 12.2 竞态条件检测

检查：

- 共享可变状态是否在并发环境下被正确保护（锁、互斥、原子操作）
- 异步操作是否存在时序依赖（先读后写、先检查后执行）
- 文件操作是否存在 TOCTOU（Time-of-Check to Time-of-Use）风险
- 定时任务、回调、事件处理器是否可能导致重复执行或死锁
- 缓存失效与数据库写入之间是否存在不一致窗口

发现竞态条件时，应要求引入适当的同步机制或幂等保护，并记录并发场景验证证据。

#### 12.3 信任边界违规检测

检查：

- 是否信任前端传入的不可信数据直接用于业务逻辑或查询
- 是否信任外部 API 返回数据未做校验即持久化
- 是否信任 Cookie/Header/QueryString 中的角色或权限信息
- 是否信任文件上传的 MIME 类型或文件名未做白名单校验
- 是否信任配置文件中的默认值未做安全审查

发现信任边界违规时，应调用 `security-best-practices` 进行专项审查，必须修复后方可进入开发审计。
