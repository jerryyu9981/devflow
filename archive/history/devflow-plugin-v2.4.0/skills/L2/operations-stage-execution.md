...
以下规则内联自 container-deployment 技能：
- Dockerfile规范：多阶段构建/层缓存优化/.dockerignore/非root用户/不使用latest标签
- Compose编排：depends_on+healthcheck控制启动顺序/dev+staging+prod override模式
- K8s部署：Deployment滚动更新+资源限制+liveness+readiness探针/Service选择决策树/Ingress TLS终止/ConfigMap+Secret外部化
- 容器安全：禁止特权容器/只读根文件系统/NetworkPolicy默认拒绝/Secret不入镜像
- 健康检查：liveness(30-60s)/readiness(5-15s)/startup(0-30s) 三种探针
- 镜像扫描：Trivy集成/Critical阻断/High 24h修复/Medium 7d修复/Low记录
...
| 2026-07-02 | VR-009: L3 速查表新增 container-deployment 容器化部署规范内联 | jerry.yu |
