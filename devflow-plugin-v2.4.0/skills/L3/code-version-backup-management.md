...
以下模板提供了超出 Git 原生备份能力的容灾方案，适用于对数据安全和业务连续性有更高要求的项目：

| 模板 | 路径 | 用途 |
|------|------|------|
| 灾难恢复预案 | `templates/DR-灾难恢复预案.md` | 定义灾难场景、恢复优先级、团队分工和恢复步骤 |
| 备份策略配置指南 | `templates/DR-备份策略配置指南.md` | 制定备份类型、频率、保留和存储策略 |
| 多地域备份方案 | `templates/DR-多地域备份方案.md` | 设计跨地域数据同步和故障切换方案 |
| 数据恢复演练流程 | `templates/DR-数据恢复演练流程.md` | 计划和执行恢复演练，评估恢复能力 |
| 备份完整性校验规范 | `templates/DR-备份完整性校验规范.md` | 定义备份校验算法、频率和失败处理流程 |

> 以上模板为可选扩展，项目应根据自身数据安全级别和合规要求选用。
...
| 日期 | 变更内容 | 变更人 |
|---|---|---|
| 2026-07-02 | 将"触发条件"从文件末尾移至"定位"之后，改为标准章节格式；添加变更记录章节 | jerry.yu |
| 2026-07-02 | VR-006: 新增容灾备份扩展模板引用（5个DR模板） | jerry.yu |
