...
6. **完成前必须强制验证（VR-2.4.1-002 增强）**：没有新鲜的验证证据不得声称实现完成。"应该能通过""我很有信心"等声明性陈述不能替代实际运行命令、查看输出、然后报告结果的过程。每项编码任务完成时，必须附带以下验证证据：
   - **实际执行的命令**（不可省略命令本身，只记录输出无效）
   - **命令输出的关键片段**（通过/失败/错误信息的原文）
   - **结论判断**（基于输出得出的通过或不通过结论，而非基于推测）
   - 验证证据格式模板：
     ```
     [验证项] <任务名称>
     [命令] <实际执行的完整命令>
     ...
     [结论] 通过/不通过 + 判断依据
     ```
   未提供完整验证证据的完成声明视为无效，必须补充证据后方可进入 `code-logic-review`。
