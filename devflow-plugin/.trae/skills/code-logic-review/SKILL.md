...
> 本维度用于防止"设计写了改名但编码没做"的设计-执行断裂问题。

检查：
- 设计文档中标记的"新建/重命名/删除"文件操作是否与实际文件系统一致
- 编码完成后，比对设计文档输出的 File Manifest 与实际文件系统的 LS/Glob 输出
- 不匹配项统一标记为 P1（不影响运行时功能但反映结构不一致），必须在开发审计前修复或记录延期理由

**执行方式**：
1. 设计文档输出时，AI 根据明确的新建/重命名/删除操作自动生成 File Manifest（预期文件清单）
2. code-logic-review 将 File Manifest 与实际文件系统的 LS/Glob 输出逐行对比
3. 不匹配项标记为 P1，记录到审查报告
