...
发布前必须对 6 个阶段（Step 0~5）的所有产出物进行逐一盘点，确认无空输出。

| 阶段 | 核心产出物 | 验证方式 |
|:----:|:----------|:--------|
| Step 0 版本规划 | 版本规划文档、迭代计划、Backlog | LS/Glob 列出 doc/version 目录核对 |
...
**盘点规则**：
- 空输出率 = 0% 才允许发布（即所有阶段的应产出物必须全部实际存在）。
- 盘点结果作为 Release Checklist 的必查项之一。
- 盘点不通过的版本不得进入发布流程，须回退对应阶段补全产出物。

---

### Release Checklist

> ⚠️ 强制规则：每项检查必须附带实际验证命令和输出片段。单纯标记"✅ 通过"视为无效。
> 执行者必须如实写明执行了哪个命令、看到了什么输出。如果某项因环境缺失无法执行，必须标记为 ⛔ **阻塞** 并说明原因，不得标记为 ✅ 通过。
>
> 示例（正确）：
> ```
...
- [ ] **全阶段产出物盘点通过** — 验证方式：LS 列出 6 个阶段目录，确认空输出率 = 0%
- [ ] **版本迭代路线图已更新** — 验证方式：grep v{版本号} doc/version/global/DevFlow-版本迭代路线图.md → 输出匹配

发布时：
- [ ] release.ps1 执行完成 — 验证命令：检查退出码 / PS 脚本执行记录
- [ ] Git Tag 已创建并推送 origin — 验证命令：`git tag -l v{版本号}` + `git ls-remote origin refs/tags/v{版本号}`
- [ ] Git Tag 已推送 backup — 验证命令：`git ls-remote {backup_url} refs/tags/v{版本号}`
...
> **目的**：验证 Release Checklist 中的每项检查是否真实可复现，防止 AI 编造验证结果。
...
9. 运维审计材料已准备好并通过。
10. 已明确允许关闭全流程。
11. **产出物存在性验证**：阶段完成前必须通过 LS/Glob 列出目标目录，确认所有产出文件实际存在，空输出（声称完成但文件不存在）不得通过。
