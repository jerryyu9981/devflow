...
Step 2 (架构设计) → 设计评审通过 → 需求架构对比审计（覆盖率≥95%） →
    ↓
Step 3 (编码开发) → 创建追溯矩阵 → 编码 → 静态检查 → 自测 → 逻辑审查 → 开发设计对比审计（覆盖率≥95%）→
    ↓
...
| 门禁指标 | 技能文件 | 阈值 | 阻断等级 | 触发阶段 |
|:---------|:---------|:-----|:--------:|:--------:|
| 圈复杂度（函数级） | code-static-quality-check | ≤ 15 | P0 | Step 3 静态检查 |
| 圈复杂度（整体） | cicd-pipeline-management | >20 阻断，>10% 函数 >15 阻断 | P0 | Step 4 CI/CD |
| 代码重复率（新代码） | code-static-quality-check | ≤ 3% | P0 | Step 3 静态检查 |
| 代码重复率 | cicd-pipeline-management | >3% P1 阻断 | P1 | Step 4 CI/CD |
| 跨层依赖 | code-static-quality-check | 禁止 | P0 | Step 3 静态检查 |
| 技术债比例 | cicd-pipeline-management | >10% P1，>20% P0 | P0/P1 | Step 4 CI/CD |
| AI Code Review | code-logic-review | P0 问题数=0 | P0 | Step 3 逻辑审查 |
| 全量回归通过率 | testing-stage-execution | ≥ 95% | P0 | Step 4 |
| 修改文件覆盖度 | testing-stage-execution | ≥ 80% | P1 | Step 4 |
| 需求架构对比覆盖 | design-stage-execution | ≥ 95% | P0 | Step 2 |
| 开发设计对比覆盖 | coding-stage-execution | ≥ 95% | P0 | Step 3 |
