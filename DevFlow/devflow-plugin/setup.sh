...
" 2>/dev/null)
    ...
    if [ "$inst_count" -eq "$ExpectedSkillCount" ]; then
        ok "Installed: $inst_count/$ExpectedSkillCount skills"
    else
        warn "Skill count mismatch: installed=$inst_count, expected=$ExpectedSkillCount"
    fi
fi
